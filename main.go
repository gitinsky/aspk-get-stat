...
func main() {
	...
	statsdInit(*cfg.statsd_host, *cfg.statsd_port)
...
func getStat() {
    ...
    defer func() {
        if e := recover(); e != nil {
            statsdIncrement("binstore2.aspk_stat.error")
            logInfo(0, sTime, "Error getting stat: %s", e.(error).Error())
        }
        statsdTiming(sTime, "binstore2.aspk_stat.done")
    }()
        ...
        for nsName, nsStat := range(*nsInfo) {
            for key, val := range(*nsStat) {
                ...
                if isIn(key, cfg.metric) {
                    num, ok := a2i(val)
                    if ok {
                        statsdGauge(num, "binstore2.aspk_stat.%s.%s", nsName, key)
                    }
                }
            }
        }
...
func a2i(str string) (int, bool) {
    ...
    switch {
        case str == "on"  : return 1, true
        case str == "off" : return 0, true
    }
